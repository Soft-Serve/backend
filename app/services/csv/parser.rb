...
module Csv
  class Parser
    ...
    def menu(row)
      Menu.find_or_create_by(name: row[2]&.value&.strip, restaurant_id: restaurant_id)
    end

    def menu_category(row)
      name = row[3]&.value ? row[3]&.value.strip : "No category"
      
      MenuCategory.find_or_create_by(name: name, menu_id: menu(row).id)
    end

    def create_allergies(item, row)
      return if row[4]&.datatype.nil?

      allergies_list = row[4].value.split(",").map(&:strip)
